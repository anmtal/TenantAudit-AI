...
html_template = '''
<!-- HOSTED MONTHLY -->
<div class="pricing-grid" id="hosted-grid-monthly" style="display: grid; max-width: 1200px; margin: 0 auto; width: 100%;">
...
def build_features(feat_list):
    return '\n            '.join(f'<li><i data-lucide="check" class="check-icon"></i> {f}</li>' for f in feat_list)
...
hosted_tiers = [
    ...
    {"name": "Corporate", "seats": 9999, "mo_price": 2499, "mo_audits": 300, "yr_price": 22499, "yr_audits": 3600, "seats_label": "Unlimited"},
    {"name": "Enterprise", "seats": 9999, "mo_price": 4999, "mo_audits": 1000, "yr_price": 44999, "yr_audits": 12000, "seats_label": "Unlimited"}
]
...
byok_tiers = [
    ...
    {"name": "Enterprise", "seats": 9999, "mo_price": 1499, "yr_price": 13499, "seats_label": "Unlimited"}
]
...
def make_hosted_cards(is_annual):
    ...
    for t in hosted_tiers:
        ...
        feats = [
            ...
            f"{t.get('seats_label', str(t['seats']) + ' Seat' + ('s' if t['seats'] > 1 else ''))}"
        ]
        if t['seats'] > 1:
...
content = re.sub(r'<div class="pricing-grid" id="hosted-grid-monthly".*?>(.*?)</div>\s*<div class="pricing-grid" id="hosted-grid-annual".*?>(.*?)</div>\s*<div class="pricing-grid" id="byok-grid".*?>(.*?)</div>', new_grids, content, flags=re.DOTALL)
